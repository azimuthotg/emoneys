...
#### มติ 13 ส.ค. 2569 — ลบ PDF v2 ทิ้งทั้งชุด

เจ้าของระบบตัดสินใจว่า **ถ้าจะพัฒนาส่วน PDF ต่อ ต้องลบ PDF v2 ทิ้งก่อน** ไม่ปลุกขึ้นมาใช้
เหตุผลคือมันตายอยู่แล้วทุกทาง การเก็บไว้มีแต่ทำให้คนรุ่นหลังหลงว่ามี PDF 2 ระบบ
แล้วไปแก้ผิดไฟล์ (ซึ่งเกิดกับผมมาแล้วในเซสชันนี้)

จุดที่ต้องลบ 5 จุด:

| ไฟล์ | ลบอะไร |
|---|---|
| `accounts/views.py` | `receipt_pdf_v2_view()`, `receipt_pdf_v2_download_view()` |
| `accounts/urls.py` | route `receipt_pdf_v2`, `receipt_pdf_v2_download` |
| `templates/accounts/receipt_pdf_v2.html` | ทั้งไฟล์ |
| `templates/accounts/receipt_detail.html` | JS กำพร้า `printReceiptV2()`, `downloadPDFV2()` |

⚠️ **หลังลบต้องพิสูจน์ว่า PDF ตัวจริงไม่ขยับ** ตามกฎเหล็กของเจ้าของระบบ
(ห้ามแตะอะไรที่ทำให้ PDF render ต่างจากเดิม) วิธีที่ใช้ได้ผลแล้วในเซสชันนี้:
เก็บไฟล์ PDF ก่อน/หลัง แล้วเทียบไบต์หลังตัด `/CreationDate`, `/ModDate`, `/ID` ทิ้ง
ต้องได้ `True` และจำนวนโทเคนข้อความต้องเท่าเดิม (ใบ `190126/0001` = 351 โทเคน)
